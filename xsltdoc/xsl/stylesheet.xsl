...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xd:doc>
    <xd:short>Controls the maximum access level displayed.</xd:short>
    <xd:detail>
  ...
  ***************************************
  ...
  <xsl:function name="xd:printParamDeclaration">
    ...
    <!-- Param name -->          
    <span class="paramName"><xsl:value-of select="$param/@name"/></span>
  ...
  <xsl:template match="xsl:stylesheet | xsl:transform" mode="outputsDetail">
    <xsl:if test="xsl:output[xd:accessMatch(.)]">
      ...
      <xsl:for-each select="xsl:output[xd:accessMatch(.)]">
        ...
        <xsl:variable name="css-zebra-class" select="if (position() mod 2 = 0) then 'even' else 'odd'"/>
        
        <div class="listItem {$css-zebra-class}">
